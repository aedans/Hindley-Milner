...
fun Expr.infer(env: Env): Pair<Subst, Type> = when (this) {
    ...
    is Expr.Apply -> {
        ...
        val substP = unify(apply(expr2Subst, expr1Type), Type.Arrow(expr2Type, typeVariable))
    ...
    is Expr.Abstract -> {
        ...
        exprSubst to Type.Arrow(apply(exprSubst, typeVariable), exprType)
    }
}

fun Env.getEnv(type: Expr.Var) = this[type]!!.let { emptySubst to it.instantiate() }
