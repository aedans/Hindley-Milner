...
/**
 * Infers a type for a [TLCExpr].
 */
fun TLCExpr.infer(env: Env): Pair<Subst, TLCMonotype> = Fix.recursive().run {
    TLCExprFFunctor.cata(this@infer, Algebra<ForTLCExprF, Eval<(Env) -> Eval<Pair<Subst, TLCMonotype>>>> {
        fun impl(env: Env) = when (val expr = it.fix()) {
            is TLCExprF.Bool -> Eval.now(emptySubst to TLCMonotypeF.bool)
